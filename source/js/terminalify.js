...
(function(window, undefined) {  
    ...
    terminalify.fn = terminalify.prototype = {
        ...
        init: function(selector, options) {
            ...
            console.log("Got selector, terminalifying.", selected);
            if (!selected) throw "No selectable element to terminalify.";
    ...
    function bindEventListeners(selector, options) {
        ...
        $("#terminal-window").click(function() {
            userActivated = true;
            if ($("#terminalCommand")) {
                console.log("Clicked in and found that thing...", $('terminalCommand'));
                $("#terminalCommand").focus();
            } else {
                console.log("Clicked in but not finding it to focus...");
            }
        });
        var count = 0;
        var lastKey = null;
    ...
    function navigateTo(options, target) {
        if (target == 'home') {
            console.log("location == ", window.location);
            window.location.href = options.rootUrl
        } else {
            window.location.href = options.rootUrl + target;
        }
    }
    ...
    function attemptExecutingCommand(options, command) {
        ...
        if (cmd == 'ls') {
            ...
            if (target.length == 1 || options.dirs.indexOf(target[1]) == -1) {
                ...
                navigateTo(options, target[1]);
            }
        } else if (options.commands[cmd]) {
            console.log("Normal command == ", cmd);
            if (typeof options.commands[cmd] == "function") {
                //call the callback function with options as 
                options.commands[cmd].call(this, options);
            } else {
    ...
    function playTypingAudio(audio) {
        var audioAsset = typeof audio == "boolean" ? 'audio/typing.mp3' : audio;
        typingAudio = new Audio(audioAsset);
        typingAudio.play();
    }

    function playKeypressAudio() {
        keypressAudio = new Audio('audio/keypress.mp3');
        keypressAudio.play();
    }
